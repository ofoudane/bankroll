...
if(VALID){
    ...
    if($result->rowCount() == 0 ){
        $answer = "<tr class='text-center table-warning' ><td colspan='4'>Vous n'avez aucun paris gagnant</td> </tr>";
    }
    else{
        $answer = "";
        while($row = $result->fetch(PDO::FETCH_BOTH)){
            $answer = $answer . "<tr class='table-success'>
                                   <td>$row[1]</td>
                                   <td>$row[2]</td>
                                   <td>$row[3]</td>
        ...
        $answer = $answer . "<tr class=\"table-secondary\">
                                <td colspan=\"2\" class=\"text-center border border-dark\">Total gain</td>
